...
calc_stats <- function(window_start, window_end, time_shift = "CHARTTIME", 
  ...
  # check input
  allowed_shifts <- c("CHARTTIME", "CHARTTIME_COLLAPSED_SHIFT_1", "CHARTTIME_COLLAPSED_SHIFT_2", "CHARTTIME_COLLAPSED_SHIFT_3","CHARTTIME_COLLAPSED_SHIFT_4", "CHARTTIME_COLLAPSED_SHIFT_5")
  # allowed_shifts <- c(allowed_shifts, "INTIME_COLLAPSED","INTIME_COLLAPSED_SHIFT_1","INTIME_COLLAPSED_SHIFT_2","INTIME_COLLAPSED_SHIFT_3","INTIME_COLLAPSED_SHIFT_4","INTIME_COLLAPSED_SHIFT_5")
  ...
  # building the query
  sql <- paste("SELECT", "DISTINCT", unit_or_category, ",", times, "COUNT(DISTINCT ITEMID) AS treatments,", "COUNT(DISTINCT SUBJECT_ID) AS patients,", "COUNT(DISTINCT CGID) AS staff, AVG(LOS_TRANS) AS avg_los", 
