    ...
    for(m in to_merge){
        other_model_flow <- unit_flow0[[m]] %>% 
            select(
                ...
                l_INFLOW,
                l_PATIENTS)
        model_flow[[i]] <- merge(model_flow[[i]], other_model_flow, 
...
outflow_models <- list()
for(i in units){
    to_model_inflow <- model_flow[[i]] %>% select(INFLOW, starts_with("from_"))
    to_model_inflow[,paste0("from_",i):=NULL]
    inflow_models[[i]] <- lm(INFLOW ~ ., to_model_inflow)
    model_flow[[i]][CHART_SHIFT == shift_types[t]] %>% 
        select(
            OUTFLOW, 
            starts_with("STAFF"), 
            PATIENTS, 
