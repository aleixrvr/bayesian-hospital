...
flow_data <- frame %>% 
  ...
  mutate(PD_RATIO = PATIENTS/STAFF) %>% 
  as.data.table() 
...
unit_flow <- lapply(unit_flow, function(X){
  ...
  X$l1_STAFF = lagpad(X$STAFF)
  X$l2_STAFF = lagpad(X$STAFF, 2)
  X$l_INFLOW = lagpad(X$INFLOW)
  X[is.na(X$PD_RATIO),]$PD_RATIO <- 0
  X$l1_PD_RATIO = lagpad(X$PD_RATIO)
  X$l2_PD_RATIO = lagpad(X$PD_RATIO, 2)
