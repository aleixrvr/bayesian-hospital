...
source("code/modeling/variations/data_prep_other_aggregation_patients_wait2.R")
library(ggplot2)
library(magrittr)
# ls()
...
change_col_names_lags <- function(other_pred_data, unit_name, other_unit){
  ...
  setnames(other_pred_data, 'l1_PATIENTS', 'PATIENTS')
...
pred_downstream_waiting <- function(pred_data, unit_name, other_unit, resources=NULL){
  ...
  if(is.null(resources)){
    return(pred_data[[add_unit_name('l1_waiting_time', other_unit)]])
  }else{
    pred_wait <- pred_data[[add_unit_name('l1_PATIENTS', other_unit)]]/max(resources, 1)
    return( pred_wait )
  }
  
  
  # staff_lags_lagged_other <- paste0(staff_lags_lagged, '_', other_unit)
  # l_patients_other <- paste0('l12_PATIENTS_', other_unit)
  # 
  # pred_data %>% 
  #   select(
