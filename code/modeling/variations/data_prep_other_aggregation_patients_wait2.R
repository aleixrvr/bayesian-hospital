...
number_of_paitents_sql <- ("SELECT * FROM `bgse-dsc.MIMIC3_V1_4.PATIENTS_DATA`")
number_of_paitents <- dbGetQuery(con, number_of_paitents_sql) %>% 
  as.data.table()
...
flow_data <- frame %>% 
    ...
    left_join(y = number_of_paitents, by=c("CHART_DATE", "CHART_SHIFT", "CURR_UNIT")) %>% 
  as.data.table() 
...
unit_flow <- lapply(unit_flow, function(X){
  X$waiting_time <- X$PATIENTS/pmax(X$STAFF, 1)
  # X[is.na(X$STAFF),]$waiting_time <- w_max
  ...
  X$l1_PATIENTS = lagpad(X$PATIENTS)
  X$l2_PATIENTS = lagpad(X$PATIENTS, 2)
  
  for(st_lag in 1:staff_lags){
    lag_name = paste0('l_STAFF_', st_lag)
    X[[lag_name]] = lagpad(X$STAFF, st_lag)
  }
