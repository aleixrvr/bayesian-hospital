...
frame <- as.data.frame(rep(seq(as.Date(dates[1,1]),as.Date(dates[1,2]), by="day"), each=6))
frame$CURR_UNIT <- units
...
# Merge stuff ====
flow_data <- frame %>% 
    left_join(y=skeleton, by=c("CHART_DATE", "CURR_UNIT")) %>%
