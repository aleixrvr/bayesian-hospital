...
sql_max <- paste0("SELECT MAX(CHARTTIME_COLLAPSED) AS MAX_CHARTTIME, ",
                  "MAX(CHARTTIME_COLLAPSED_SHIFT_1) AS MAX_CHARTTIME_SHIFT_1, ",
                  "MAX(CHARTTIME_COLLAPSED_SHIFT_2) AS MAX_CHARTTIME_SHIFT_2, ",
                  "MAX(CHARTTIME_COLLAPSED_SHIFT_3) AS MAX_CHARTTIME_SHIFT_3, ",
                  "MAX(CHARTTIME_COLLAPSED_SHIFT_4) AS MAX_CHARTTIME_SHIFT_4, ",
                  "MAX(CHARTTIME_COLLAPSED_SHIFT_5) AS MAX_CHARTTIME_SHIFT_5 ",
                  "FROM MIMIC3_V1_4.CHARTEVENTS_DEPTS_CATS_TS_COLLAPSED")
time_shifts_max <- dbGetQuery(con, sql_max)

sql_min <- paste0("SELECT MIN(CHARTTIME_COLLAPSED) AS MIN_CHARTTIME, ",
                  "MIN(CHARTTIME_COLLAPSED_SHIFT_1) AS MIN_CHARTTIME_SHIFT_1, ",
                  "MIN(CHARTTIME_COLLAPSED_SHIFT_2) AS MIN_CHARTTIME_SHIFT_2, ",
                  "MIN(CHARTTIME_COLLAPSED_SHIFT_3) AS MIN_CHARTTIME_SHIFT_3, ",
                  "MIN(CHARTTIME_COLLAPSED_SHIFT_4) AS MIN_CHARTTIME_SHIFT_4, ",
                  "MIN(CHARTTIME_COLLAPSED_SHIFT_5) AS MIN_CHARTTIME_SHIFT_5 ",
                  "FROM MIMIC3_V1_4.CHARTEVENTS_DEPTS_CATS_TS_COLLAPSED")
