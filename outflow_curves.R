...
outflow_do <- function(unit, unit_flow_data, outflow_models, r = c(-20,20), w_max = w_max_param){
  # set units in the ICU to be examined
  do_unit <- unit
  all_units <- names(unit_flow_data)
  units <- all_units[all_units != unit]
  
  sel_ressources = unique(pmax(median(unit_flow[[unit]]$STAFF,na.rm = TRUE) + r, 0))
  sel_ressources = sel_ressources[1]:sel_ressources[2]
  ...
  for(i in units){
    ...
    pred_waiting[[i]] <- 1 / theta_thres(predict(outflow_models[[i]],  pred_data) - inflow, w_max)
    colnames(pred_data) <- orig_cols
  }
  # estimate waiting time curves by varying staff in the unit of interest 
  to_plot <- data.frame(ressources=integer(), outflow=numeric(), unit=character(), 
                        do_unit=character())
  for(ressources in sel_ressources){
    df <- data.frame(matrix(unlist(pred_waiting), ncol=length(pred_waiting), byrow=FALSE))
    df <- cbind(ressources, df)
    colnames(df) <- c("STAFF", paste0("l1_waiting_time_",names(pred_waiting)))
    outflow <- mean(predict(outflow_models[[unit]], df))
    to_plot <- rbind(to_plot, data.frame(ressources, outflow, unit, do_unit))
  }
  
  # estimate waiting time curves by varying staff in the downstream units
  for(downstream_unit in units){
    pred_waiting_old <- pred_waiting[[downstream_unit]]
    ...
    for(ressources in sel_ressources){
      ...
      pred_waiting[[downstream_unit]] <- 1 / theta_thres(predict(outflow_models[[downstream_unit]],  
      ...
      colnames(df) <- c("STAFF", paste0("l1_waiting_time_",names(pred_waiting)))
      outflow <- mean(predict(outflow_models[[unit]], df))
      to_plot <- rbind(to_plot, data.frame(ressources, outflow, unit=downstream_unit, do_unit))
    }
    
    pred_waiting[[downstream_unit]] <- pred_waiting_old
  }
...
#     see <- outflow_do(i, unit_flow, outflow_models)
...
to_plot <- data.frame(ressources=integer(), outflow=numeric(), unit=character(), do_unit=character())
